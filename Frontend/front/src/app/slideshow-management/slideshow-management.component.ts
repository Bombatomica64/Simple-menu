...
@Component({
  ...
  imports: [
    ...
    InputTextModule, InputNumberModule, CheckboxModule, FileUploadModule,
    ConfirmDialogModule, ToastModule, CalendarModule, DragDropModule,
    ProgressBarModule
  ],
        ...
        <div class="header-actions">
          ...
          @if (activeSlideshow()) {
            <button
              pButton
              type="button"
              [label]="activeSlideshowStatus() ? 'Disattiva Slideshow' : 'Attiva Slideshow'"
              [icon]="activeSlideshowStatus() ? 'pi pi-stop' : 'pi pi-play'"
              [class]="activeSlideshowStatus() ? 'p-button-danger' : 'p-button-success'"
      ...
      <p-dialog
        ...
        <div class="slides-management">
          <div class="upload-section">
            ...
            <p class="upload-help">Seleziona una o più immagini da aggiungere allo slideshow</p>
...
export class SlideshowManagementComponent implements OnInit, OnDestroy {
  ...
  saveSlideshow() {
    ...
    if (editing) {
      ...
      this.slideshowService.updateSlideshow(editing.id, this.slideshowForm).subscribe({
        next: () => {
  ...
  onImageUpload(event: any) {
    ...
    if (response.success && this.selectedSlideshow()) {
      this.slideshowService.addSlideToSlideshow(
        this.selectedSlideshow()!.id,
        ...
        next: () => {
          ...
          this.selectedSlideshow.set(
            this.allSlideshows().find(s => s.id === this.selectedSlideshow()?.id) || null
          );
        },
        error: (error) => {
  ...
  onMultipleImageUpload(event: any) {
    try {
      ...
      this.messageService.add({
        severity: 'error',
        summary: 'Errore',
        detail: 'Errore nel caricamento delle immagini'
      });
    }
  }
