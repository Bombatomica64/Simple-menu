...
const GRADIENT_PRESETS = [
  ...
  { name: 'Ocean', value: 'linear-gradient(135deg, #667db6 0%, #0082c8 0%, #0082c8 0%, #667db6 100%)' },
...
const PAGE_OPTIONS = [
  { label: 'Pagina Pasta (/home)', value: 'pasta' },
  { label: 'Menu Completo (/menu)', value: 'menu' },
  { label: 'Pagina Submit (/submit)', value: 'submit' }
];
...
export class BackgroundPaletteComponent {
  ...
  backgroundConfigDeleted = output<string>(); // page name
  ...
  currentBackgroundValue = computed(() => {
    ...
    switch (type) {
      ...
      case 'image':
        return this.selectedImageUrl() ? `url('${this.selectedImageUrl()}')` : '';
      default:
        return '';
    }
  });
  ...
  openBackgroundDialog(page?: string) {
  ...
  async loadBackgroundConfigs() {
    ...
    try {
      const response = await this.http.get<BackgroundConfig[]>(`${this.apiUrl}/api/backgrounds`).toPromise();
  ...
  onImageUpload(event: any) {
    ...
    this.http.post<any>(`${this.apiUrl}/api/backgrounds/upload`, formData)
      .subscribe({
        next: (response) => {
  ...
  async saveBackgroundConfig() {
    ...
    if (!page || !value) {
      this.messageService.add({
        severity: 'warn',
        summary: 'Attenzione',
        detail: 'Seleziona una pagina e configura uno sfondo'
      });
      return;
    }
    ...
    try {
      ...
      const response = await this.http.post<BackgroundConfig>(`${this.apiUrl}/api/backgrounds`, config).toPromise();

      // Update local state
      this.backgroundConfigs.update(configs => {
        const existingIndex = configs.findIndex(c => c.page === page);
        if (existingIndex >= 0) {
      ...
      this.backgroundConfigUpdated.emit(response!);
      this.closeDialog();

      this.messageService.add({
        severity: 'success',
        summary: 'Successo',
        detail: `Sfondo configurato per ${page}`
      });
    } catch (error) {
  ...
  async deleteBackgroundConfig(page: string) {
    ...
    this.loading.set(true);
    try {
      await this.http.delete(`${this.apiUrl}/api/backgrounds/page/${page}`).toPromise();

      // Update local state
      this.backgroundConfigs.update(configs =>
        configs.filter(c => c.page !== page)
      );

      this.backgroundConfigDeleted.emit(page);

      this.messageService.add({
        severity: 'success',
        summary: 'Successo',
        detail: `Configurazione sfondo eliminata per ${page}`
      });
    } catch (error) {
  ...
  getPageLabel(page: string): string {
    const option = this.pageOptions.find(opt => opt.value === page);
